...
from preprocessing.encoding.datasets.patched_wsi_inference import (
    PatchedWSIInference,
    PatchInference,
)
...
class CellSegmentationInference:
    ...
    def __load_inference_transforms(self):
        ...
        if "normalize" in transform_settings:
            ...
            mean = (0.5, 0.5, 0.5)
            std = (0.5, 0.5, 0.5)
        self.inference_transforms = T.Compose([T.ToTensor()])
    ...
    def process_patches(
        ...
        with torch.no_grad():
            for batch in tqdm.tqdm(dataloader):
                ...
                instance_map, instance_types, tokens = (
                    self.get_cell_predictions_with_tokens(predictions)
                )
        ...
        for i, (name, instance_types) in enumerate(zip(patch_names, instance_types_gb)):
            instance_segmentation_geogson[name] = {
                str(key): value for key, value in instance_types_gb[i].items()
            }
        ...
        for name, instance_map in zip(patch_names, instance_map_gb):
            cv2.imwrite(
                os.path.join(save_path, "pred_binary_masks", name),
                self.get_binary_mask(np.array(instance_map)),
            )
            cv2.imwrite(
                os.path.join(save_path, "pred_semantic_masks", name),
                self.semantic_map_rgb(
                    np.array(instance_map), instance_segmentation_geogson[name]
                ),
            )
            self.barplot(instance_segmentation_geogson[name]).savefig(
                os.path.join(save_path, "mask_barplots", name)
            )
    ...
    def barplot(self, mask_json, title="Cell Type Distribution", figsize=(6, 4)):
        # Count each cell type
        class_dist = {}
        for _, value in mask_json.items():
            cell_type = value.get("type", None)
            if cell_type is not None:
                class_dist[cell_type] = class_dist.get(cell_type, 0) + 1

        if not class_dist:
            # If class_dist is empty, show an empty plot with a message
        ...
        # Create barplot
        plt.figure(figsize=figsize)
        bars = plt.barh(categories, values, color=palette)
    ...
    def get_cell_predictions_with_tokens(
        ...
        (
            ...
            predictions, magnification=magnification, nuclei_type_map=TYPE_NUCLEI_DICT
        )
...
if __name__ == "__main__":
    ...
    if command.lower() == "process_patches":
        ...
        stain_normalize = configuration["is_stain_normalize"] == 1
        if stain_normalize:
            print("Applying Stain Normalization")
        else:
            print("Not Using Stain Normalization")
        cell_segmentation.process_patches(
            patch_path,
            batch_size=batch_size,
        ...
        for i, wsi_path in enumerate(wsi_filelist):
            ...
            cell_segmentation.logger.info(
                f"File {i + 1}/{len(wsi_filelist)}: {wsi_name}"
            )
            wsi_file = WSI(
                name=wsi_name,
                patient=wsi_name,

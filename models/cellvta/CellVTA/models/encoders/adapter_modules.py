...
def deform_inputs(x):
    ...
    level_start_index = torch.cat((spatial_shapes.new_zeros(
        (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
    reference_points = get_reference_points([(h // 4, w // 4),
                                             (h // 8, w // 8),
                                             (h // 16, w // 16)], x.device)
...
class Extractor(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, feat):
            ...
            if self.with_cffn:
                query = query + self.drop_path(self.ffn(self.ffn_norm(query), H*2, W*2))
            return query
...
class Injector(nn.Module):
    ...
    def forward(self, query, reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        # query = rearrange(query, "b (h w) c -> b h w c", h=H, w=W)
        return query
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W):
        ...
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          ...
                          level_start_index=deform_inputs1[2], H=H, W=W)
...
class SpatialPriorModule(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False):
        ...
        self.conv4 = nn.Sequential(*[
            nn.Conv2d(4*inplanes, 4 * inplanes, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(4 * inplanes),
            nn.ReLU(inplace=True),
